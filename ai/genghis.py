...
def getAction(state, time_left=None):
    ...
    for key in state.board.continents:
        ...
        for i in state.board.continents[key].territories:
            ...
            continents[key].append(curr_territory.name)

    if state.turn_type == 'PreAssign':
        selected_action = decide_presign(state, actions, continents)

    if state.turn_type == 'Attack':
        selected_action = decide_attack(state, actions, continents)

    if state.turn_type == 'Place' or state.turn_type == 'Fortify' or state.turn_type == 'PrePlace':
        selected_action = decide_place(state, actions, continents)
...
def decide_place(state: RiskState, actions, continents):
    ...
    for key in continents:
        ...
        if get_ownership_percentage_of_continent(state, continents[key]) == 1:
            ...
            border_territories = get_border_territories(state, continents[key])
...
def decide_attack(state: RiskState, actions, continents):
...
def get_border_territories(state, continent):
    border_territories = []
    for territory in continent:
